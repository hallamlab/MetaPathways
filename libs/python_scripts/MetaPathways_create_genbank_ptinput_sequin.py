...
def  write_ptinput_files(output_dir_name, contig_dict, sample_name, nucleotide_seq_dict, protein_seq_dict):
     ...
     try:
        ...
        zeropffile = open(output_dir_name + "/0.pf", 'w')
...
if __name__ == '__main__':
    ...
    if not options.gbk_file and not options.sequin_file and not options.ptinput_file:
       parser.error('At least a genbank or a sequin or ptools input must be specified')
    ...
    process_gff_file(options.gff_file, output_files, nucleotide_seq_dict, protein_seq_dict) 
